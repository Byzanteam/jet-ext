defmodule JetExt.Absinthe.Relay.Node do
  @moduledoc """
  ...
  This will create an interface, `:node` that expects one field, `:node_id`, be
  ...
  This creates a field, `:node`, with one argument: `:node_id`. This is expected to
  ...
  ID as the value of its `:node_id` field. Using the `node` macro, you can easily do
  ...
  By default, type of `:node_id` field is `ID`. But you can pass custom type in `:id_type` attribute:
  ...
  def resolve_with_global_id(
    ...
    case from_global_id(global_id, res.schema) do
      {:ok, result} ->
  ...
  def resolve_with_global_id(res, _unused) do
    res
  end
  ...
  @spec to_global_id(binary() | atom(), binary(), Absinthe.Schema.t()) :: global_id()
  @doc """
  ...
  @spec global_id_translator(Absinthe.Schema.t()) :: term()
  @doc false
  ...
  @spec global_id_resolver(Absinthe.Resolution.t(), term()) ::
          Absinthe.Resolution.t()
  @doc false
  ...
  * Matches `:id` out of the value.
  ...
  def default_id_fetcher(%{id: id}, _info), do: to_string(id)
  def default_id_fetcher(_id, _info), do: nil
end
